                            ...
                            const std::vector<float>& zData, float lineWidth)
{
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        newSeries.vertices.insert(newSeries.vertices.end(), {
            xData[i], yData[i], z, colorR, colorG, colorB
        });
    }
...
void PlotView::renderAxisNumbers(QPainter& painter)
{
    ...
    for (int i = 0; i <= numTicks; ++i) {
        ...
        if (screenPos.z() > -1.0f && screenPos.z() < 1.0f) { // Within view frustum
...
void PlotView::renderInteractionMode(QPainter& painter)
{
    ...
    switch (m_interactionMode) {
        ...
        case ZOOM_MODE:
            ...
            break;
        case PAN_MODE:
            modeText = "PAN (T) - Drag to pan view";
            break;
    }
    ...
    // Also show keyboard shortcuts in bottom-left corner
